		...
		foreach ($event->resources as $resource) {
			if ($resource instanceof NodeResource && $event->action instanceof ShareAction) {
				...
				if (($event->action->filesSharingPermissions !== null && ($event->action->filesSharingPermissions & ~$resource->getNodePermissions()) !== 0)
					|| (($unifiedSharingPermissionsAsFilesSharingPermission = $event->action->unifiedSharingPermissionsAsFilesSharingPermission()) !== null && ($unifiedSharingPermissionsAsFilesSharingPermission & ~$resource->getNodePermissions()) !== 0)) {
