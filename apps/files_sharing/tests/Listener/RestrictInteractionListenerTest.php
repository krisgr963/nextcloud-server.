...
final class RestrictInteractionListenerTest extends TestCase {
	...
	public function testNodeResourceShareActionIncreasePermission(): void {
		...
		foreach ([$fileNode, $folderNode] as $node) {
			foreach ([
				new ShareAction(Constants::PERMISSION_READ | Constants::PERMISSION_SHARE | Constants::PERMISSION_UPDATE),
				new ShareAction(null, [NodeReadSharePermissionType::class, ReshareSharePermissionType::class, NodeUpdateSharePermissionType::class]),
			] as $action) {
				$event = new RestrictInteractionEvent($this->user->getUID(), $this->user, [new NodeResource($node->getId(), $this->user->getUID(), $node)], $action, []);
	...
	public function testNodeResourceShareActionIncreasePermissionFileDelete(): void {
		...
		$this->assertEquals('You cannot share "/foo.txt" with more permission than you have yourself.', $event->isInteractionRestricted());
