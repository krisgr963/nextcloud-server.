...
final class ApiV1ControllerTest extends AbstractSharingManagerTests {
	...
	protected function listShares(ShareAccessContext $accessContext, ?string $filterSourceTypeClass, ?string $filterSourceTypeValue, ?string $lastShareID, ?int $limit): array {
		return $this->executeRequest($accessContext, function (ApiV1Controller $controller) use ($filterSourceTypeClass, $filterSourceTypeValue, $lastShareID, $limit): DataResponse {
			if ($limit !== null) {
				/** @psalm-suppress ArgumentTypeCoercion */
				return $controller->listShares($filterSourceTypeClass, $filterSourceTypeValue, $lastShareID, $limit);
			}

			/** @psalm-suppress ArgumentTypeCoercion */
			return $controller->listShares($filterSourceTypeClass, $filterSourceTypeValue, $lastShareID);
		});
	}
}
