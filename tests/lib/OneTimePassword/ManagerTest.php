...
class ManagerTest extends TestCase {
	...
	/**
	 * Sets up the dispatcher to respond to GetOneTimePasswordEvent events
	 *
	 * @param string $providerId a provider id to use or 'invalid' to not create a provider (but still the necessary event mocks)
	 * @return void
	 * @throws \PHPUnit\Framework\MockObject\Exception
	 */
	private function setupGetOneTimePasswordProvidersEvent(string $providerId, int $expectCount): void {
		$this->dispatcher
			->expects($this->exactly($expectCount))
			->method('dispatchTyped')->with($this->callback(function (GetOneTimePasswordProvidersEvent $event) use ($providerId) {
				if ($providerId !== 'invalid') {
					$mockProvider = $this->createMock(IOneTimePasswordProvider::class);
					$mockProvider->method('getProviderId')->willReturn($providerId);
					$event->addProvider($mockProvider);
				}
				return true;
			}));
	}
	...
	public static function dataTestCreate() {
		return [
			...
			[5, 'mock', 'receiver', 'pw', self::parseDateTime('2026-07-01 02:30:12')],
			[2, 'invalid', 'receiver', null, self::parseDateTime('2026-07-01 16:30:12')],
			[12, 'mock', 'receiver', 'pw', null],
		];
	}
	...
	public function testCreate($otpId, $providerId, $recipient, $passwordHash, $expiration): void {
		...
		$this->setupGetOneTimePasswordProvidersEvent($providerId, 1);
