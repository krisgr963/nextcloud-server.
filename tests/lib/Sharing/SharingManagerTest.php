...
final class SharingManagerTest extends AbstractSharingManagerTests {
	 ...
	 */
	#[\Override]
	protected function listShares(ShareAccessContext $accessContext, ?string $filterSourceTypeClass, ?string $filterSourceTypeValue, ?string $lastShareID, ?int $limit): array {
		/** @psalm-suppress ArgumentTypeCoercion */
		return array_map(fn (Share $share): array => $share->format($this->registry, Server::get(IFactory::class), Server::get(IURLGenerator::class), Server::get(IUserManager::class)), $this->manager->listShares($accessContext, $filterSourceTypeClass, $filterSourceTypeValue, $lastShareID, $limit));
	}
}
